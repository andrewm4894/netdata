...
- **ChangeFinder Scores** (`changefinder.scores`): This chart (turned off by default but available with `show_scores: true`) shows the percentile of the score that is output from the ChangeFinder library. So a high observed score is more likley to be a valid changepoint worth exploring, even more so when multiple charts of dimensions have high changepoint scores at the same time. It is this percentile based socre that is used to drive the "Flags" chart, so this chart is turned off by default to make it easier to just focus on periods where the scores have crossed a dynamic percentile based threshold.
